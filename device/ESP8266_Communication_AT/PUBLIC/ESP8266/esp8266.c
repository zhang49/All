...
u8 ESP8266_ReadLocal()
{
	u8 clientid;
	if(!ESP8266_ReadLocalData(&clientid,10))return 0;
	if(clientid==0xff) return 0;
	do
	{
		u8 i=0;
		...
		char temp[3][50];
		u8 restartflag=0;
		printf("Client id:%d , Data:%s\r\n",clientid,sRecvBuf[clientid].rData);
		//测试
		//ESP8266_test();
		outjson=json_object();
		injson=json_loads(sRecvBuf[clientid].rData, JSON_DECODE_ANY,NULL);
