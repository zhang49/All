...
void comm_esp_now_recv_data(){
	...
	u16 hum,temp;
	case ReplyDht22:
		temp=esp_now_recv_buf.data[1]*100+esp_now_recv_buf.data[2];
		if(esp_now_recv_buf.data[0]=='-'){
			temp*=-1;
		}
		hum=esp_now_recv_buf.data[3]*100+esp_now_recv_buf.data[4];
		comm_temperature_value_write_api(temp);
		comm_humidity_value_write_api(hum);
		ESPNOW_DBG("ReplyDht22 Temp :%3d,Hum :%d",temp,hum);
		break;
	case ReplyRelay:
		DisplayMac("ReplyRelay Mac",esp_now_recv_buf.mac_addr);
		comm_relay_status_set_inner_api(esp_now_recv_buf.mac_addr,esp_now_recv_buf.data[0]);
		break;
	}
}
