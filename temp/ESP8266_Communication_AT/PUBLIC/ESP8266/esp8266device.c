...
u8 ESP8266_CmdIsSuccess(char *expect,u16 tenMsTimes)
{
	...
	{
		while(rdQueue.head==rdQueue.rear && tc<255)//等待数据
		{
			delay_ms(10);
			tc+=1;
		}
...
u8 ESP8266_ReadIpdData(u8 *id)
{
	...
	*id=0xff;
	...
	if(rdQueue.rear!=cursor && needReadIpdLength&0x3fff)
	do
	{
		u16 len;
		...
		if (sRecvBuf[cid].singleLen == 0)
		{
			...
			if(cursor+NET_HEAD_SIZE>rdQueue.rear)break;
			sRecvBuf[cid].singleLen = rdQueue.data[(++cursor) % USART_SLOT_SIZE];
			sRecvBuf[cid].singleLen<<=8;
			sRecvBuf[cid].singleLen|=rdQueue.data[(++cursor) % USART_SLOT_SIZE];
			
			needReadIpdLength-=NET_HEAD_SIZE;
		...
		//读取该包
		len=needReadIpdLength&0x3fff;
		while (rdQueue.rear!=cursor && len > 0 && sRecvBuf[cid].singleLen>0)
		{
			cursor++;
			sRecvBuf[cid].singleLen--;
			len--;
			sRecvBuf[cid].rData[sRecvBuf[cid].cursor++]= rdQueue.data[cursor % USART_SLOT_SIZE];
		}			
		needReadIpdLength=len;
		if(len!=0)needReadIpdLength|=0x8000;
		if(sRecvBuf[cid].singleLen==0)
		{
			//printf("get over\r\n");
			sRecvBuf[cid].rData[sRecvBuf[cid].cursor]= 0;
			sRecvBuf[cid].cursor=0;
			rdQueue.head=cursor;
			//单次最多取1条数据
			*id=cid;
				//printf("needreadlength is :%d\r\n",needReadIpdLength);
				//printf("singleLen is :%d\r\n",sRecvBuf[cid].singleLen);
...
//单路模式，收到的消息带+IPD,前缀，存到sRecvBuf[clientid].rData中
...
//	u16 tc=0;
//	u16 len;
//	if(rdQueue.rear==rdQueue.head)
//	{	
//		return 0;//计时溢出，没有数据
//	}
//	else
//	{
//		//暂定使用非透传的needReadIpdLength，sRecvBuf[0]
//		u16 cursor=rdQueue.head;
//		u8 srbcur=sRecvBuf[0].rear%SCR_SLOT_MAX;
//		if(needReadIpdLength==0)
//		{
//			if(rdQueue.rear<cursor+2)return 0;
//			needReadIpdLength=rdQueue.data[(++cursor)%USART_SLOT_SIZE];//高位
//			needReadIpdLength<<=8;
//			needReadIpdLength|=rdQueue.data[(++cursor)%USART_SLOT_SIZE];
//		}
//		while(needReadIpdLength && cursor!=rdQueue.rear)
//		{
//			++cursor;
//			sRecvBuf[0].rData[srbcur][sRecvBuf[0].cursor++]=rdQueue.data[cursor];
//			needReadIpdLength--;
//		}
//		if(needReadIpdLength==0)
//		{
//				sRecvBuf[0].rData[srbcur][sRecvBuf[0].cursor++]=0;
//				//printf("recv SingleData is:%s\r\n",(char *)sRecvBuf[cid].rData[sRecvBuf[cid].rear%SCR_SLOT_MAX]);		
//				if(sRecvBuf[0].rear==0xfe)
//				{
//					sRecvBuf[0].rear-=(sRecvBuf[0].head-sRecvBuf[0].head%SCR_SLOT_MAX);
//					sRecvBuf[0].head%=SCR_SLOT_MAX;
//				}
