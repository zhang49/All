...
md5_test(void) {
	...
	MD5Digest(encrypt, os_strlen((char *) encrypt), decrypt);
	...
	u8 *key = "key";
	HMAC_MD5(encrypt, os_strlen((char *) encrypt), key, output);
	debug("Digest: ");
	for (i = 0; i < 16; i++) {
		if (output[i] < 0x10) {
			debug("%d", 0);
	...
	HMAC_MD5(encrypt, os_strlen((char *) encrypt), key16, output);
	debug("Digest 16: ");
	for (i = 0; i < 16; i++) {
		if (output[i] < 0x10) {
			debug("%d", 0);
	...
	HMAC_MD5(encrypt, os_strlen((char *) encrypt), key64, output);
	debug("Digest 64: ");
	for (i = 0; i < 16; i++) {
		if (output[i] < 0x10) {
			debug("%d", 0);
