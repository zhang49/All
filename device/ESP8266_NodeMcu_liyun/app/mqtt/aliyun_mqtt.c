...
#define HASH_KEY	DEVICE_SECRET
	...
	u8 output[16];
	u8 *p = output_pass;
	u8 *pStr = (u8 *)os_malloc(os_strlen(HASH_STR)+os_strlen(input_timestamp_3));
 ...
 */
	...
	// mqtt host
	os_strncpy(cst_al_mqtt.host, MQTT_HOST, BUF_SIZE);
	// mqtt port
	cst_al_mqtt.port = MQTT_PORT;
	// mqtt client id，拼接timestamp
	os_sprintf(cst_al_mqtt.client_id, MQTT_CLIENT_ID, timestamp_str);
	// mqtt username
	os_strncpy(cst_al_mqtt.username, MQTT_USERNAME, BUF_SIZE);
	// mqtt password
	gen_mqtt_password(timestamp_str, cst_al_mqtt.password);
	// mqtt keepalive
	cst_al_mqtt.keepalive = 120;

	debug("mqtt host:%s\r\n"
			"mqtt port:%d\r\n"
			"mqtt client id:%s\r\n"
			"mqtt username:%s\r\n"
			"mqtt password:%s\r\n", cst_al_mqtt.host, cst_al_mqtt.port,
			cst_al_mqtt.client_id, cst_al_mqtt.username,
			cst_al_mqtt.password);
