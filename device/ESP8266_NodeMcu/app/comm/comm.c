...
static comm_def comm_operator[]={
		...
		{"GetSafeConfig",		"Reply_GetSafeConfig",		NULL,				comm_wifi_safe_read,		NULL},
...
static void statusTimerCb(void *arg){
	...
	if(temperature_read_tick==8){
		...
		syn_state.ray=comm_sensor_ray_value_read();
...
const char *ICACHE_FLASH_ATTR get_res_type(void *client){
	...
	char *data=(c_sign==CLIENT_IS_HTTP?(((http_connection *)client)->body.data):(((MqttUserData *)client)->data));
...
int ICACHE_FLASH_ATTR comm_expect_ret(void *client){
	...
	case SYN_CONTROL:
		{
			cJSON *root=cJSON_Parse(c_sign==CLIENT_IS_HTTP?(((http_connection *)client)->body.data):(((MqttUserData *)client)->data));
	...
	case NORMAL_WIFI_SCAN:
		if(Timer->tickcount==1)comm_wifi_api_scan_start();
			...
			cJSON *retroot=comm_wifi_api_scan();
			if(retroot!=NULL){
				os_timer_disarm(&Timer->timer);
				return send_ret_json(client,retroot,EC_Normal);
